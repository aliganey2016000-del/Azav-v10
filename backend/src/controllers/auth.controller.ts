  ...
  static async register(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!parsed.success) {
        res.status(400).json({
          success: false,
          error: { code: 'VALIDATION_ERROR', message: 'Please provide valid registration details.' },
        });
        return;
      }
      ...
      const result = await AuthService.registerUser({
        ...
        universityId: resolvedApplicantType === ApplicantType.INDEPENDENT ? null : universityId,
        phone,
      });
