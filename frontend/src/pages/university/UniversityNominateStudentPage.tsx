...
type FormState = {
  ...
  trainingPurpose: string; batchId: string;
...
const emptyForm: FormState = {
  ...
  trainingPurpose: '', batchId: '', documents: {},
};
...
export const UniversityNominateStudentPage: React.FC = () => {
  ...
  const loadBatches = () => {
    ...
    AdminApiService.getTrainingBatches(user.universityId)
      .then((items) => setTrainingBatches(items))
      .catch(() => setTrainingBatches([]))
  ...
  const openAdd = () => {
    setForm({ ...emptyForm, batchId: lastBatchId });
    setStep(1);
    setEditingId(null);
    setFileError(null);
    setSubmitError(null);
  ...
  const openEdit = (s: AdminStudent) => {
    setForm({
      ...
      trainingPurpose: '', batchId: s.batch?._id || '', documents: {},
    });
    setStep(1);
    setEditingId(s._id);
    setFileError(null);
  ...
  const filtered = useMemo(
    () => students.filter(s => `${s.firstName} ${s.lastName} ${s.studentNumber} ${s.specialty} ${s.batch?.batchNumber || ''} ${s.batch?.name || ''}`.toLowerCase().includes(query.toLowerCase())),
    [students, query]
  );
  ...
  const createAndSelectBatch = async () => {
    if (!user?.universityId) {
      setSubmitError('Your account is not linked to a university.');
      return;
    }
    ...
    try {
      ...
      change('batchId', created._id);
      setLastBatchId(created._id);
      setShowNewBatch(false);
      setNewBatch({ batchNumber: '', name: '', intakeDate: '' });
      setSuccessMessage(`Batch ${created.batchNumber} created and selected.`);
  ...
  const submit = async () => {
    ...
    if (!form.batchId) {
      setSubmitError('Select or create a Batch No for this student.');
      setStep(3);
      return;
    }
    ...
    <div className="space-y-5 pb-8">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full sm:max-w-md"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400"/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search by name, ID, batch or program..." className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-xs outline-none focus:ring-2 focus:ring-blue-500"/></div>
        ...
        <div className="max-h-[94vh] w-full max-w-3xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
          ...
          <div className="p-5 sm:p-6">
            ...
            {step === 3 && <div className="space-y-4">
              <Panel title="Nomination Batch" subtitle="Students arriving together should use the same Batch No. This will make later billing and tracking easier." tone="violet">
                <div className="space-y-3">
                  <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
                    <Field label="Batch No *">
                      <select
                        ...
                        onChange={e => {
                          change('batchId', e.target.value);
                          if (e.target.value) setLastBatchId(e.target.value);
                        }}
                        className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-bold text-slate-800 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100 disabled:bg-slate-100"
                      >
                        <option value="">{batchLoading ? 'Loading batches...' : 'Select existing batch'}</option>
                        {trainingBatches.filter(batch => batch.status === 'OPEN').map(batch => (
                    ...
                    <button
                      ...
                      className="min-h-11 rounded-xl border border-violet-200 bg-white px-4 text-xs font-extrabold text-violet-700 hover:bg-violet-50"
                    >
                  ...
                  {form.batchId && (() => {
                    const selected = trainingBatches.find(batch => batch._id === form.batchId);
                    return selected ? (
                      <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-[11px] font-semibold text-emerald-800">
                        Selected: <span className="font-black">{selected.batchNumber}</span> · {selected.name}
                        {selected.intakeDate ? ` · Intake ${new Date(selected.intakeDate).toLocaleDateString()}` : ''}
                      </div>
                    ) : null;
                  })()}
                      ...
                      <div className="grid gap-3 sm:grid-cols-3">
                        <Field label="Batch No">
                          <Input
                            ...
                            onChange={value => setNewBatch(current => ({ ...current, batchNumber: value }))}
                        ...
                        <Field label="Batch Name">
                          <Input
                            value={newBatch.name}
                            onChange={value => setNewBatch(current => ({ ...current, name: value }))}
                        ...
                        <Field label="Intake Date">
                          <DateInput
                            ...
                            onChange={value => setNewBatch(current => ({ ...current, intakeDate: value }))}
                          />
                        </Field>
                      </div>
                      <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <p className="text-[10px] font-semibold leading-4 text-slate-500">
                          Leave Batch No blank and the system will generate one from the university code and year.
                        </p>
                        <button
                          type="button"
                          onClick={() => void createAndSelectBatch()}
                          disabled={creatingBatch}
                          className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-violet-600 px-4 text-xs font-extrabold text-white disabled:opacity-50"
                        >
              ...
              <Panel title="Training Request" subtitle="Requested clinical training details" tone="violet">
                <Grid>
                  <Field label="Clinical Specialty"><Input value={form.requestedSpecialty} onChange={v=>change('requestedSpecialty',v)} placeholder="e.g. General Surgery"/></Field>
                  ...
                  <Field label="Training Purpose" wide><textarea value={form.trainingPurpose} onChange={e=>change('trainingPurpose',e.target.value)} className="min-h-24 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100" placeholder="Brief purpose of training"/></Field>
                </Grid>
              </Panel>
            </div>}
            ...
            <div className="mt-6 flex items-center justify-between border-t border-slate-100 pt-5"><button onClick={() => step === 1 ? reset() : setStep(step-1)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-bold text-slate-600 hover:bg-slate-50">{step === 1 ? 'Cancel' : 'Back'}</button>{step < 5 ? <button onClick={() => {
              ...
              if (step === 3 && !form.batchId) {
                setSubmitError('Select an existing Batch No or create a new batch before continuing.');
                return;
              }
              setSubmitError(null);
              setStep(step+1);
            }} className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-violet-600 px-5 py-2.5 text-xs font-extrabold text-white shadow-md">Next <ChevronRight className="h-4 w-4"/></button> : <button onClick={submit} disabled={submitting || !form.fullName.trim() || !form.studentId.trim() || !form.email.trim() || !form.batchId || (!editingId && form.password.length < 8) || (editingId && form.password.length > 0 && form.password.length < 8)} className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 px-5 py-2.5 text-xs font-extrabold text-white shadow-md disabled:cursor-not-allowed disabled:opacity-50">{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4"/>} {editingId ? 'Save Changes' : 'Submit Nomination'}</button>}</div>
          </div>
        </div>
      </div>}
    </div>
