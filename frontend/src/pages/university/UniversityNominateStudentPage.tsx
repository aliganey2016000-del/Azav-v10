...
export const UniversityNominateStudentPage: React.FC = () => {
  ...
  const reset = () => { setForm(emptyForm); setStep(1); setOpen(false); setEditingId(null); };
  const openAdd = () => { setForm(emptyForm); setStep(1); setEditingId(null); setOpen(true); };
  const openEdit = (s: RealTrainee) => {
    setForm({
      ...
      preferredStartDate: s.startDate, preferredEndDate: s.endDate, trainingPurpose: '',
    });
    setStep(1);
    setEditingId(s.id);
    setOpen(true);
  ...
  const submit = () => {
    ...
    if (editingId) {
      setStudents(RealDataStore.updateTrainee(editingId, {
        ...
        startDate: form.preferredStartDate, endDate: form.preferredEndDate, durationWeeks: weeks,
      }));
      reset();
      return;
    }
