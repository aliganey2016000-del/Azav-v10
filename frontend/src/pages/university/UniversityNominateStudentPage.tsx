...
import { Award, CheckCircle2, ChevronRight, Clock3, Eye, GraduationCap, Pencil, Plus, Search, UserPlus, Users, X, XCircle } from 'lucide-react';
...
const emptyForm: FormState = {
  ...
  preferredStartDate: '', preferredEndDate: '', trainingPurpose: '',
};
...
export const UniversityNominateStudentPage: React.FC = () => {
  ...
  const reset = () => { setForm(emptyForm); setStep(1); setOpen(false); };
    ...
    <div className="space-y-5 pb-8">
      <section className="overflow-hidden rounded-3xl bg-gradient-to-r from-blue-900 via-blue-700 to-teal-500 p-6 text-white shadow-lg sm:p-7">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button onClick={() => setOpen(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-extrabold text-blue-700 shadow-lg transition hover:-translate-y-0.5"><Plus className="h-4 w-4" /> Nominate New Student</button>
        </div>
      </section>
      ...
      {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-3 backdrop-blur-[2px] sm:p-6" onMouseDown={e => { if (e.target === e.currentTarget) reset(); }}>
        <div className="max-h-[94vh] w-full max-w-3xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
          <div className="sticky top-0 z-10 bg-gradient-to-r from-blue-700 via-indigo-600 to-violet-600 p-5 text-white sm:p-6"><div className="flex items-start justify-between gap-4"><div className="flex gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/15"><GraduationCap className="h-6 w-6"/></div><div><h2 className="text-xl font-black">Nominate a Student</h2><p className="mt-1 text-xs text-blue-100">Fill in student details for clinical training</p></div></div><button onClick={reset} className="rounded-xl bg-white/10 p-2 hover:bg-white/20"><X className="h-5 w-5"/></button></div></div>
          <div className="p-5 sm:p-6">
            <div className="mb-6 grid grid-cols-4 gap-2">{['Student Details','Academic Info','Training Request','Review & Submit'].map((label, i) => <div key={label} className="text-center"><div className={`mx-auto flex h-8 w-8 items-center justify-center rounded-full text-xs font-black ${step >= i+1 ? 'bg-blue-600 text-white' : 'border border-slate-300 bg-white text-slate-500'}`}>{i+1}</div><div className={`mt-1 hidden text-[10px] font-bold sm:block ${step === i+1 ? 'text-blue-700' : 'text-slate-400'}`}>{label}</div></div>)}</div>
...
function Status({status}:{status:RealTrainee['applicationStatus']}){const map:any={SUBMITTED:['Pending','bg-amber-100 text-amber-700'],UNDER_REVIEW:['In Review','bg-blue-100 text-blue-700'],ACCEPTED:['Approved','bg-emerald-100 text-emerald-700'],REJECTED:['Rejected','bg-rose-100 text-rose-700']};const [label,cls]=map[status]||map.SUBMITTED;return <span className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold ${cls}`}>{label}</span>}
