    ...
    <div className="space-y-6">
      ...
      {loading ? <LoadingState message="Loading university list..." /> : error ? <ErrorState message={error} onRetry={fetchUniversities} /> : universities.length === 0 ? <EmptyState title="No universities found" description="Create a new institution or adjust search filters." /> : (
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-xs overflow-hidden">
          ...
          <div className="md:hidden divide-y divide-slate-100">{universities.map(uni => <div key={uni._id} className="p-4 space-y-3"><div className="flex items-start justify-between gap-3"><Link to={`/admin/universities/${uni._id}`} className="font-bold text-slate-900 text-base">{uni.name}</Link><div className="flex items-center gap-2">{canManage && <button type="button" onClick={() => openEditModal(uni)} aria-label={'Edit ' + uni.name} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-[11px] font-bold text-teal-700 bg-teal-50 border border-teal-200 rounded-lg active:bg-teal-100"><Pencil className="w-3.5 h-3.5" /><span>Edit</span></button>}<StatusBadge status={uni.status} /></div></div><div className="text-[11px] text-slate-500 flex justify-between gap-3"><span>{uni.code}</span><span className="text-right">{uni.city || 'Mogadishu'}, {(uni as any).country || 'Somalia'}</span></div></div>)}</div>
