          ...
          {isDevelopment && (
            <div className="relative">
              <button
                ...
                className="flex items-center space-x-2 px-3 py-1.5 bg-teal-50 border border-teal-200 rounded-lg text-xs font-medium text-teal-800 hover:bg-teal-100 transition shadow-xs"
              ...
              {roleMenuOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-200 py-1.5 z-50 text-xs">
                  <div className="px-3 py-1.5 font-semibold text-slate-400 border-b border-slate-100 uppercase text-[10px]">
                    Development: Switch Demo Persona
                  </div>
                  {Object.entries(DEMO_USERS).map(([roleKey, demo]) => (
                    <button
                      key={roleKey}
                      onClick={() => {
                        switchDemoRole(roleKey as UserRole);
                        setRoleMenuOpen(false);
                      }}
                      className={`w-full text-left px-3 py-2 hover:bg-slate-50 transition flex items-center justify-between ${
                        currentRole === roleKey ? 'bg-teal-50/60 font-semibold text-teal-900' : 'text-slate-700'
                      }`}
                    >
